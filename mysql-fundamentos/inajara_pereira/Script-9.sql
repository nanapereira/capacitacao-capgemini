...
create procedure gerar_comissao(in data_inicial date,     -- parametro de entrada
		...
		begin              
	        ...
	        declare buscar_pedido cursor for
	        	select codigo_venda, total, id_vendedor
	        		from venda v 
	        			where data_venda between data_inicial and data_final
	       					and v.total > 0;
	
	        -- Faço aqui um tratamento para o banco não executar o loop quando ele terminar evitando que retorne qualquer erro	
	       declare continue handler for sqlstate '02000' set fimloop  = 1;    
	        ...
	        open buscar_pedido;    
	            ...
	            -- Aqui verifico que se o loop terminou e saio do loop
	            if fimloop  = 1 then 
	              leave  vendas;
	            	end if;
		        	...
		        	fetch buscar_pedido into venda, total_venda, vendedor;
			            ...
			            if(vl_comissao > 0 ) then
			                ...
			                set valor_total_comissao = ((total_venda * vl_comissao) / 100);
			                	update     venda 
			                set vl_comissao = valor_total_comissao where     codigo_venda = venda;
			                	commit;
			
			            elseif (vl_comissao = 0 ) then
			                	update     venda 
			                set     vl_comissao = 0 where     codigo_venda = venda;
			                	commit;
			            else 
			                set vl_comissao = 1;
			                set valor_total_comissao = ((total_venda * vl_comissao) / 100);
	                ...
	                -- atribuo o total de vendas para a variável desaída

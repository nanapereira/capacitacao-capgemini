...
-- Não exibir a coluna as 'IDADE DO LIVRO'
...
-- Ordernar por autor e ano_lancamento DESC ?
...
-- Field 'data_edicao' doesn't have a default value
...
select count(distinct autor) as 'QT Autores' from livro;
...
alter table LIVRO modify VALOR decimal(5,2);
...
update livro set VALOR = 9.99 where nome = 'Dom Quixote';

select sum(valor)/count(*) as 'Media de preços' from livro;

select nome, data_edicao, 
	case when data_edicao > "2015-01-01" 
	then "O livro é de no máximo dois anos"
...
select nome, case 
 ...
 end,sum(valor)/count(*) from LIVRO;